...
namespace MechSuitsApi.Areas.Accounts
{
  //  [Authorize]
    ...
    public class BankJournalController : ControllerBase
    {
        ...
        public BankJournalController(AppDBContext context, IHttpContextAccessor httpContextAccessor, IUriService uriService)
        {
        ...
        public void updatepicturepath(string id, string imgname)
        {
            ...
            strsql = @"UPDATE BankJournal_Header SET IMAGE='" + imgname + "' where VoucherId='" + id + "'";
        ...
        {
            return await _context.BankJournal_Header.ToListAsync();
        }
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get1([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.BankJournal_Header.Where(m => (m.Checked == "0" && m.VoucherType == "BRV"))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.BankJournal_Header.Where(m => m.Checked == "0" && m.VoucherType == "BRV").CountAsync();
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get3([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.BankJournal_Header.Where(m => (m.VoucherId.Contains(title) && (m.Checked == "0" && m.VoucherType == "BRV")))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.BankJournal_Header.Where(m => (m.VoucherId.Contains(title) && (m.Checked == "0" && m.VoucherType == "BRV"))).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Header>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get2([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var route = Request.Path.Value;
            var validFilter = new CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter(filter.PageNumber, filter.PageSize);
            var pagedData = await _context.BankJournal_Header.Where(m => (m.Checked == "1" && m.VoucherType == "BRV"))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.BankJournal_Header.Where(m => (m.Checked == "1" && m.VoucherType == "BRV")).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Header>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        }
        [HttpGet]
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get6([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.BankJournal_Header.Where(m => (m.VoucherId.Contains(title) && (m.Checked == "1" && m.VoucherType == "BRV")))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.BankJournal_Header.Where(m => (m.VoucherId.Contains(title) && (m.Checked == "1" && m.VoucherType == "BRV"))).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Header>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        ...
        {
            var m = await _context.BankJournal_Header.FindAsync(id);
        ...
        public string create(M_Header b)
        {
            ...
            b.ChequeNumber = b.ChequeNumber == null ? "" : b.ChequeNumber;
            ...
            //M_Chart.Code = (getNext(M_Chart.Company_Code, M_Chart.Level1Code, M_Chart.Level2Code, M_Chart.Level3Code));
        ...
        public void update(M_Header m)
        {
            ...
            m.ChequeNumber = m.ChequeNumber == null ? "" : m.ChequeNumber;
            string journalcode = "";
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> GetLists(string purpose, string type)
        {
            ...
            return await _context.BankJournal_Header.Where(m => m.Checked == purpose && m.VoucherType == type).ToListAsync();
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> GetLists(string purpose)
        {
            ...
            return await _context.BankJournal_Header.Where(m => m.Checked == purpose).ToListAsync();
        ...
        public string postunpost(string purpose)
        {
            ...
            general.UpdateTable("BankJournal_Header", "Checked", ls_value, WhereCondition);
            //general = new GeneralFunctions(c.V_connection);
