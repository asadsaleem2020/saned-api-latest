...
namespace MechSuitsApi.Areas.Accounts
{
   // [Authorize]
    ...
    public class ExpenseController : ControllerBase
    {
        ...
        public ExpenseController(AppDBContext context, IHttpContextAccessor httpContextAccessor, IUriService uriService)
        {
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get()
        {
            ...
            return await _context.Expense_Header.ToListAsync();
        }
        [HttpGet]
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get1([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.Expense_Header.Where(m => m.Checked == "0")
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.Expense_Header.Where(m => m.Checked == "0").CountAsync();
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get3([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.Expense_Header.Where(m => (m.VoucherId.Contains(title) && m.Checked == "0"))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.Expense_Header.Where(m => (m.VoucherId.Contains(title) && m.Checked == "0")).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Header>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get2([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var route = Request.Path.Value;
            var validFilter = new CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter(filter.PageNumber, filter.PageSize);
            var pagedData = await _context.Expense_Header.Where(m => m.Checked == "1")
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.Expense_Header.Where(m => m.Checked == "1").CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Header>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        }
        [HttpGet]
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get6([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.Expense_Header.Where(m => (m.VoucherId.Contains(title) && m.Checked == "1"))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.Expense_Header.Where(m => (m.VoucherId.Contains(title) && m.Checked == "1")).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Header>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        ...
        public void updatepicturepath(string id, string imgname)
        {
            ...
            strsql = @"UPDATE Expense_Header SET IMAGE='" + imgname + "' where VoucherId='" + id + "'";
        ...
        {
            var m = await _context.Expense_Header.FindAsync(id);
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> GetLists(string purpose)
        {
            ...
            return await _context.Expense_Header.Where(m => m.Checked == purpose).ToListAsync();
        ...
        public string postunpost(string purpose)
        {
            ...
            general.UpdateTable("Expense_Header", "Checked", ls_value, WhereCondition);
            //general = new GeneralFunctions(c.V_connection);
