...
namespace MechSuitsApi.Areas.Accounts
{
   // [Authorize]
    ...
    public class BankPaymentController : Controller
    {
        ...
        public BankPaymentController(AppDBContext context, IHttpContextAccessor httpContextAccessor, IUriService uriService)
        {
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get1([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.BankJournal_Header.Where(m => (m.Checked == "0" && m.VoucherType == "BPV"))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.BankJournal_Header.Where(m => m.Checked == "0" && m.VoucherType == "BPV").CountAsync();
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get3([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.BankJournal_Header.Where(m => (m.VoucherId.Contains(title) && (m.Checked == "0" && m.VoucherType == "BPV")))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.BankJournal_Header.Where(m => (m.VoucherId.Contains(title) && (m.Checked == "0" && m.VoucherType == "BPV"))).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Header>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get2([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var route = Request.Path.Value;
            var validFilter = new CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter(filter.PageNumber, filter.PageSize);
            var pagedData = await _context.BankJournal_Header.Where(m => (m.Checked == "1" && m.VoucherType == "BPV"))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.BankJournal_Header.Where(m => (m.Checked == "1" && m.VoucherType == "BPV")).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Header>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        }
        [HttpGet]
        ...
        public async Task<ActionResult<IEnumerable<M_Header>>> Get6([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.BankJournal_Header.Where(m => (m.VoucherId.Contains(title) && (m.Checked == "1" && m.VoucherType == "BPV")))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.BankJournal_Header.Where(m => (m.VoucherId.Contains(title) && (m.Checked == "1" && m.VoucherType == "BPV"))).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Header>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
