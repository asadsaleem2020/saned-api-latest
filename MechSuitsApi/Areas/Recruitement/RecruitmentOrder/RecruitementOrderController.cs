...
//        {
//            return await _context.RecruitementOrder.ToListAsync();
//        }
...
//            var querySQL = "SELECT (Select Name from RCustomer where Code = Client) as Client, " +
...
//            if (type == "agency")
//            {
//                querySQL = "SELECT (Select Name from RCustomer where Code = Client) as Client, " +
//                "(Select Name from Country where Code = Country) as Country, " +
//                "(Select Name from professions where Code = Profession)as Profession, " +
//                "(select Name from Agents where Code = IDNumber) as IDNumber, Company_Code, " +
//                "HijriDate, Date, VisaType, VisaNumber, VisaDate, Religion, Type, Experience, Age, " +
...
//        // GET: api/Level2/5
//        [HttpGet("{id}")]
//        public async Task<ActionResult<M_RecruitementOrder>> Getm(string id)
//        {
...
namespace MechSuitsApi.Areas.Recruitement.RecruitmentOrder
{
    ...
    public class RecruitementOrderController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<M_RecruitementOrder>>> Get3(string type, string condition, [FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            var route = Request.Path.Value;
            var validFilter = new CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter(filter.PageNumber, filter.PageSize);
            var querySQL = "SELECT (Select Name from RCustomer where Code = Client) as Client,OrderStatus,TimelineStatus, " +
                  "(Select Name from Country where Code = Country) as Country, " +
                  "(Select Name from professions where Code = Profession)as Profession, " +
                  "(select Name from Agents where Code = IDNumber) as IDNumber, Company_Code, " +
                  "HijriDate, Date, VisaType, VisaNumber, VisaDate, Religion, Type, Experience, Age, " +
            ...
            if (type == "agency")
            {
                querySQL = "SELECT (Select Name from RCustomer where Code = Client) as Client,OrderStatus,TimelineStatus, " +
                "(Select Name from Country where Code = Country) as Country, " +
                "(Select Name from professions where Code = Profession)as Profession, " +
                "(select Name from Agents where Code = IDNumber) as IDNumber, Company_Code, " +
                "HijriDate, Date, VisaType, VisaNumber, VisaDate, Religion, Type, Experience, Age, " +
