...
namespace MechSuitsApi.Areas.Purchase.Controllers
{
    ...
    public class PurchaseReturnController : Controller
    {
        ...
        public PurchaseReturnController(AppDBContext context, IHttpContextAccessor httpContextAccessor, IUriService uriService)
        {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<M_PRHeader>>> Get()
        {

            return await _context.PRHEADER.ToListAsync();
        }

        [HttpGet]
        [Route("UNPOSTED/chunks")]
        public async Task<ActionResult<IEnumerable<M_PRHeader>>> Get1([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            // return await _context.Area.ToListAsync();
            var route = Request.Path.Value;
            var validFilter = new CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter(filter.PageNumber, filter.PageSize);
            var pagedData = await _context.PRHEADER.Where(m => m.STATUS == false)
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.PRHEADER.Where(m => m.STATUS == false).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_PRHeader>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        }
        [HttpGet]
        [Route("search")]
        public async Task<ActionResult<IEnumerable<M_PRHeader>>> Get3([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.PRHEADER.Where(m => (m.INVOICECODE.Contains(title) && m.STATUS == false))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.PRHEADER.Where(m => (m.INVOICECODE.Contains(title) && m.STATUS == false)).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_PRHeader>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        ...
        [HttpGet]
        [Route("POSTED/chunks")]
        public async Task<ActionResult<IEnumerable<M_PRHeader>>> Get2([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            // return await _context.Area.ToListAsync();
            var route = Request.Path.Value;
            var validFilter = new CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter(filter.PageNumber, filter.PageSize);
            var pagedData = await _context.PRHEADER.Where(m => m.STATUS == true)
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.PRHEADER.Where(m => m.STATUS == true).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_PRHeader>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        }
        [HttpGet]
        [Route("posted/search")]
        public async Task<ActionResult<IEnumerable<M_PRHeader>>> Get6([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.PRHEADER.Where(m => (m.INVOICECODE.Contains(title) && m.STATUS == true))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.PRHEADER.Where(m => (m.INVOICECODE.Contains(title) && m.STATUS == true)).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_PRHeader>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        ...
        public void updatepicturepath(string id, string imgname)
        {
            ...
            strsql = @"UPDATE PRHEADER SET IMAGE='" + imgname + "' where VoucherId='" + id + "'";
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<M_PRHeader>> Get(string id)
        {

            var m = await _context.PRHEADER.FindAsync(companycode, id);
        ...
        public ActionResult<IEnumerable<M_PRDetail>> GetDetail(string voucherid)
        {
        ...
        [HttpPost]
        public void create(M_PRHeader H)
        {
        ...
        [HttpPut]
        [Route("update")]
        public void update(M_PRHeader H)
        {
            H.COMPANY_CODE =  companycode ;
            H.CREATED_BY = "";
            H.CREATED_ON = DateTime.Now;
        ...
        public async Task<ActionResult<IEnumerable<M_PRHeader>>> GetLists(string purpose)
        {
            string where = "1=1";
            var flag = false;
            if (purpose == "1")
            ...
            return await _context.PRHEADER.Where(m => m.STATUS == flag).ToListAsync();
        ...
        public string postunpost(string purpose)
        {
            ...
            general.UpdateTable("PRHEADER", "STATUS", ls_value, WhereCondition);
