...
namespace MechSuitsApi.Areas.Hr.Controllers
{
    [Route("api/HrSetting")]
    [ApiController]
    public class HrSettingController : ControllerBase
    {
        ...
        public HrSettingController(AppDBContext context, IHttpContextAccessor httpContextAccessor, IUriService uriService)
        {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<M_HrSetting>>> GetAS_Acclevel2()
        {
            return await _context.HrSetting.ToListAsync();
        }
        //[HttpGet]
        //[Route("chunks")]
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<M_HrSetting>> GetM_Level2(string id)
        {
            var m_Level2 = await _context.HrSetting.FindAsync(id);
        ...
        [HttpPut]
        [Route("update")]
        public async Task<IActionResult> update(M_HrSetting m_Level2)
        {
            ...
            try
            {
                var obj = new M_HrSetting();
                obj = await _context.HrSetting.FindAsync(m_Level2.Code);

                if (obj != null)
                {

                    obj.IpAddress = m_Level2.IpAddress;
                    obj.WorkingHours = m_Level2.WorkingHours;
                    obj.WorkPeriod = m_Level2.WorkPeriod;
                    obj.WorkingDays = m_Level2.WorkingDays;
                    obj.DaysOfAbsense = m_Level2.DaysOfAbsense;
                    obj.ExtraHours = m_Level2.ExtraHours;
                    obj.HoursOfAbsense = m_Level2.HoursOfAbsense;
