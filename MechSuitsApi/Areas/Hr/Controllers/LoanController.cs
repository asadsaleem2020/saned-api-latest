...
namespace MechSuitsApi.Areas.Hr.Controllers
{
   // [Authorize]
    ...
    public class LoanController : Controller
    {
        ...
        public LoanController(AppDBContext context, IHttpContextAccessor httpContextAccessor, IUriService uriService)
        {
        ...
        // GET: api/Departments
        [HttpGet]
        public async Task<ActionResult<IEnumerable<M_Loan>>> GetAS_Acclevel2()
        {
            return await _context.HR_LOAN.ToListAsync();
        }

        [HttpGet]
        [Route("chunks")]
        public async Task<ActionResult<IEnumerable<M_Loan>>> Get1([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            // return await _context.Area.ToListAsync();
            var route = Request.Path.Value;
            var validFilter = new CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter(filter.PageNumber, filter.PageSize);
            var pagedData = await _context.HR_LOAN
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.HR_LOAN.CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Loan>(pagedData, validFilter, totalRecords, uriService, route);
        ...
        [HttpGet]
        [Route("search")]
        public async Task<ActionResult<IEnumerable<M_Loan>>> Get3([FromQuery] CoreInfrastructure.ItemInformation.ItemInformation.PaginationFilter filter)
        {
            ...
            var pagedData = await _context.HR_LOAN.Where(m => m.EMPLOYEE_CODE.Contains(title))
                    .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
                    .Take(validFilter.PageSize)
                    .ToListAsync();
            var totalRecords = await _context.HR_LOAN.Where(m => m.EMPLOYEE_CODE.Contains(title)).CountAsync();
            var pagedReponse = PaginationHelper.CreatePagedReponse<M_Loan>(pagedData, validFilter, totalRecords, uriService, route);
            return Ok(pagedReponse);
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<M_Loan>> GetM_Level2(string id)
        {
            var m_Level2 = await _context.HR_LOAN.FindAsync(id);
        ...
        public async Task<ActionResult<IEnumerable<M_Loan>>> GetLevel2WithLevel1(string id)
        {

            var m_Level2 = await _context.HR_LOAN.Where(x => x.DOCUMENT_NO == id).ToListAsync();
        ...
        public async Task<IActionResult> update(M_Loan m_Level2)
        {
            ...
            {
                var obj = new M_Loan();
                obj = await _context.HR_LOAN.FindAsync(m_Level2.DOCUMENT_NO);
                ...
                { 
                    ...
                    obj.INSTALLMENT = m_Level2.INSTALLMENT;
                    obj.Start_Date = m_Level2.Start_Date;
        ...
        [HttpPost]
        public async Task<ActionResult<M_Loan>> create(M_Loan m_Level2)
        {
            m_Level2.DOCUMENT_NO = getUpdateMasterCount();
            _context.HR_LOAN.Add(m_Level2);
            await _context.SaveChangesAsync();
        ...
        public string getUpdateMasterCount()
        {
            ...
            strsql = @"SELECT ISNULL(MAX( CAST(DOCUMENT_NO AS BIGINT  ))  ,0) +1  AS code FROM HR_LOAN";
        ...
        [HttpDelete("{id}")]
        public async Task<ActionResult<M_Loan>> DeleteM_Level2(string id)
        {
            var m_Level2 = await _context.HR_LOAN.FindAsync(id);
            if (m_Level2 == null)
            {
            ...
            _context.HR_LOAN.Remove(m_Level2);
            await _context.SaveChangesAsync();
