  ...
  public  class D_DB 
    {
        ...
        public List<M_NewUser> GetList(string whereCondition)
        {
            string strsql;
            ...
            sda.Fill(_dt);
            List<M_NewUser> list = null;
            if (_dt.Rows.Count > 0)
            {
                list = new List<M_NewUser>();
                foreach (DataRow sqldr in _dt.Rows)
                {
                    var fields = new M_NewUser();
        ...
        public M_NewUser GetSingle(string whereCondition)
        {
            ...
            M_NewUser fields = null;
            if (sqldr.Read())
            {
                fields = new M_NewUser();
        ...
        public long getUpdateMasterCount()
        {
            ...
            string strsql;
            long no = 0;
            strsql = @"SELECT ISNULL(CAST( MAX(Code) AS BIGINT) ,0) +1  AS code FROM App_Users";
        ...
        public void Set_b(int Mode, M_NewUser m)
        {
