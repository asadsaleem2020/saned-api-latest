...
namespace CoreInfrastructure.GeneralSetting.MeasuringUnit
{
    public class D_DB
    {
        ...
        public List<M_MeasuringUnit> GetList(string whereCondition)
        {
            ...
            strsql = "select * from MeasuringUnit where   " + whereCondition;
            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable _dt = new DataTable();
            sda.Fill(_dt);
            List<M_MeasuringUnit> list = null;
            if (_dt.Rows.Count > 0)
            {
                list = new List<M_MeasuringUnit>();
                foreach (DataRow sqldr in _dt.Rows)
                {
                    var fields = new M_MeasuringUnit();
        ...
        public M_MeasuringUnit GetSingle(string whereCondition)
        {
            ...
            M_MeasuringUnit fields = null;
            if (sqldr.Read())
            {
                fields = new M_MeasuringUnit();
            //    fields.ID = int.Parse(sqldr["ID"].ToString());
                fields.Code = sqldr["Code"].ToString();
                fields.Name = sqldr["Name"].ToString();
                fields.Locked = bool.Parse(sqldr["Locked"].ToString());
        ...
        public string getUpdateMasterCount()
        {
            ...
            strsql = @"SELECT ISNULL(MAX( CAST(CODE AS BIGINT  ))  ,0) +1  AS code FROM MeasuringUnit";
        ...
        public void Set_b(int Mode, M_MeasuringUnit m)
        {
            ...
            strsql = "Execute SP_MeasuringUnit " + Mode.ToString() + ",";
