...
namespace CoreInfrastructure.GeneralSetting.Color
{
    public class D_DB
    {
        ...
        public List<M_Color> GetList(string whereCondition)
        {
            ...
            strsql = "select * from Color where   " + whereCondition;
            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable _dt = new DataTable();
            sda.Fill(_dt);
            List<M_Color> list = null;
            if (_dt.Rows.Count > 0)
            {
                list = new List<M_Color>();
                foreach (DataRow sqldr in _dt.Rows)
                {
                    var fields = new M_Color();
        ...
        public M_Color GetSingle(string whereCondition)
        {
        ...
        public string getUpdateMasterCount()
        {
            ...
            strsql = @"SELECT ISNULL(MAX( CAST(CODE AS BIGINT  ))  ,0) +1  AS code FROM Color";
