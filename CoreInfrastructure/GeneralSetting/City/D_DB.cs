...
namespace CoreInfrastructure.GeneralSetting.City
{
    public class D_DB
    {
        ...
        public List<M_City> GetList(string whereCondition)
        {
            ...
            strsql = "select * from City where   " + whereCondition;
            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable _dt = new DataTable();
            sda.Fill(_dt);
            List<M_City> list = null;
            if (_dt.Rows.Count > 0)
            {
                list = new List<M_City>();
                foreach (DataRow sqldr in _dt.Rows)
                {
                    var fields = new M_City();
        ...
        public M_City GetSingle(string whereCondition)
        {
        ...
        public string getUpdateMasterCount()
        {
            ...
            strsql = @"SELECT ISNULL(MAX( CAST(CODE AS BIGINT  ))  ,0) +1  AS code FROM City";
        ...
        public void Set_b(int Mode, M_City m)
        {
            ...
            strsql = "Execute SP_City " + Mode.ToString() + ",";
