...
namespace CoreInfrastructure.GeneralSetting.Area
{
    public class D_DB
    {
        ...
        public List<M_Area> GetList(string whereCondition)
        {
            ...
            strsql = "select * from Area where   " + whereCondition;
            ...
            if (_dt.Rows.Count > 0)
            {
                list = new List<M_Area>();
                foreach (DataRow sqldr in _dt.Rows)
                {
                    var fields = new M_Area();
        ...
        public M_Area GetSingle(string whereCondition)
        {
        ...
        public string getUpdateMasterCount()
        {
            ...
            strsql = @"SELECT ISNULL(MAX( CAST(CODE AS BIGINT  ))  ,0) +1  AS code FROM Area";
        ...
        public void Set_b(int Mode, M_Area m)
        {
            ...
            strsql = "Execute SP_Area " + Mode.ToString() + ",";
