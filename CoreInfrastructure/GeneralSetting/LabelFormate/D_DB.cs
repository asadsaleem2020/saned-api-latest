...
namespace CoreInfrastructure.GeneralSetting.LabelFormate
{
    public class D_DB
    {
        ...
        public List<M_LabelFormate> GetList(string whereCondition)
        {
            ...
            strsql = "select * from LabelFormate where   " + whereCondition;
            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable _dt = new DataTable();
            sda.Fill(_dt);
            List<M_LabelFormate> list = null;
            if (_dt.Rows.Count > 0)
            {
                list = new List<M_LabelFormate>();
                foreach (DataRow sqldr in _dt.Rows)
                {
                    var fields = new M_LabelFormate();
        ...
        public M_LabelFormate GetSingle(string whereCondition)
        {
            ...
            M_LabelFormate fields = null;
            if (sqldr.Read())
            {
                fields = new M_LabelFormate();
            //    fields.ID = int.Parse(sqldr["ID"].ToString());
                fields.Code = sqldr["Code"].ToString();
                fields.Name = sqldr["Name"].ToString();
                fields.Locked = bool.Parse(sqldr["Locked"].ToString());
        ...
        public string getUpdateMasterCount()
        {
            ...
            strsql = @"SELECT ISNULL(MAX( CAST(CODE AS BIGINT  ))  ,0) +1  AS code FROM LabelFormate";
