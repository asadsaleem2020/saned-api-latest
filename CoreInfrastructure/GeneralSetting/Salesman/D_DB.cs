...
namespace CoreInfrastructure.GeneralSetting.Salesman
{
    public class D_DB
    {
        ...
        public List<M_Salesman> GetList(string whereCondition)
        {
            ...
            strsql = "select * from Salesman where   " + whereCondition;
            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable _dt = new DataTable();
            sda.Fill(_dt);
            List<M_Salesman> list = null;
            if (_dt.Rows.Count > 0)
            {
                list = new List<M_Salesman>();
                foreach (DataRow sqldr in _dt.Rows)
                {
                    var fields = new M_Salesman();
        ...
        public M_Salesman GetSingle(string whereCondition)
        {
            ...
            M_Salesman fields = null;
            if (sqldr.Read())
            {
                fields = new M_Salesman();
            //    fields.ID = int.Parse(sqldr["ID"].ToString());
                fields.Code = sqldr["Code"].ToString();
                fields.Name = sqldr["Name"].ToString();
                fields.Locked = bool.Parse(sqldr["Locked"].ToString());
        ...
        public string getUpdateMasterCount()
        {
            ...
            strsql = @"SELECT ISNULL(MAX( CAST(CODE AS BIGINT  ))  ,0) +1  AS code FROM Salesman";
        ...
        public void Set_b(int Mode, M_Salesman m)
        {
            ...
            strsql = "Execute SP_Salesman " + Mode.ToString() + ",";
