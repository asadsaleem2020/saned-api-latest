...
namespace CoreInfrastructure.GeneralSetting.Country
{
    public class D_DB
    {
        ...
        public List<M_Country> GetList(string whereCondition)
        {
            ...
            strsql = "select * from country where   " + whereCondition;
            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable _dt = new DataTable();
            sda.Fill(_dt);
            List<M_Country> list = null;
            if (_dt.Rows.Count > 0)
            {
                list = new List<M_Country>();
                foreach (DataRow sqldr in _dt.Rows)
                {
                    var fields = new M_Country();
        ...
        public M_Country GetSingle(string whereCondition)
        {
            ...
            M_Country fields = null;
            if (sqldr.Read())
            {
                fields = new M_Country();
            //    fields.ID = int.Parse(sqldr["ID"].ToString());
                fields.Code = sqldr["Code"].ToString();
                fields.Name = sqldr["Name"].ToString();
                fields.Locked = bool.Parse(sqldr["Locked"].ToString());
        ...
        public string getUpdateMasterCount()
        {
            ...
            strsql = @"SELECT ISNULL(MAX( CAST(CODE AS BIGINT  ))  ,0) +1  AS code FROM country";
        ...
        public void Set_b(int Mode, M_Country m)
        {
            ...
            strsql = "Execute SP_country " + Mode.ToString() + ",";
