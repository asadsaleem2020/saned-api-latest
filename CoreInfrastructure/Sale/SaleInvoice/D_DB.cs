...
namespace CoreInfrastructure.Sale.SaleInvoice
{
    public class D_DB
    {
        ...
        public List<M_SaleHeader> GetList(string whereCondition)
        {
            ...
            strsql = "select * from SALE_INVOICE_HEADER where   " + whereCondition;

            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            List<M_SaleHeader> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_SaleHeader>();
                foreach (DataRow sqldr in dt.Rows)
                {
                    var fields = new M_SaleHeader
                    {
        ...
        public M_SaleHeader GetSingleContract(string whereCondition)
        {
...
from sale_contract_header where   " + whereCondition;
            ...
            M_SaleHeader fields = null;
            fields = new M_SaleHeader();
            if (sqldr.Read())
            {
        ...
        public List<M_SaleDetail> GetDetailRowForEdit(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            ...
            strsql = "select * from SALEINVOICEDETAIL where   " + whereCondition;
            if (con.State == ConnectionState.Closed)
            {
                con.Open();
            }
            ...
            List<M_SaleDetail> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_SaleDetail>();
                foreach (DataRow row in dt.Rows)
                {
                    var fields = new M_SaleDetail
                    {
        ...
        public M_SaleHeader GetSingle(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
...
(select name from currency  where id = currency) as currency_name,
(select name from PaymentMode where value = LTRIM(RTRIM(Payment_Mode))) as Payment_Mode_name,
(select name from salestype where value = LTRIM(RTRIM(Sales_type))) as Sales_type_name from SALEINVOICEHEADER  where   " + whereCondition;
            sqldr = dp.ExecuteReader(con, strsql);
            M_SaleHeader fields = null;
            fields = new M_SaleHeader();
            if (sqldr.Read())
        ...
        public string getUpdateMasterCount(string yearmonth, M_SaleHeader M)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
            ...
            strsql = @" Select  isnull(max(cast(right(INVOICECODE, 4) as bigint)),10000)	  as code   From SALEINVOICEHEADER   
        ...
        public string getUpdateMasterCount(string WHERE)
        {
            ...
            strsql = @"SELECT ISNULL(MAX(CAST(INVOICECODE AS bigint)),0)   as code   FROM SALEINVOICEHEADER WHERE " + WHERE;
        ...
        public int Delete_Voucher_for_Edit_mode(string where)
        {
            ...
            strsql = "delete  from SALEINVOICEDETAIL where " + where;
            SqlCommand cmd = new SqlCommand(strsql, con);
        ...
        public void Set_c(int Mode, M_SaleHeader m)
        {
            ...
            strsql = "Execute SP_SALEINVOICEHEADER " + Mode.ToString() + ",";
        ...
        public void Set_d(int Mode, M_SaleDetail m)
        {
            ...
            strsql = "Execute SP_SALEINVOICEDETAIL " + Mode.ToString() + ",";
