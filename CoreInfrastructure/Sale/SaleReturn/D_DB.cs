...
namespace CoreInfrastructure.Sale.SaleReturn
{
    public class D_DB
    {
        ...
        public List<M_SRHeader> GetList(string whereCondition)
        {
            ...
            strsql = "select * from SRHEADER where   " + whereCondition;

            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            List<M_SRHeader> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_SRHeader>();
                foreach (DataRow sqldr in dt.Rows)
                {
                    var fields = new M_SRHeader
                    {
        ...
        public M_SRHeader GetSingleContract(string whereCondition)
        {
...
from sale_contract_header where   " + whereCondition;
            ...
            M_SRHeader fields = null;
            fields = new M_SRHeader();
            if (sqldr.Read())
            {
        ...
        public List<M_SRDetail> GetDetailRowForEdit(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            ...
            strsql = "select * from SRDETAIL where   " + whereCondition;
            if (con.State == ConnectionState.Closed)
            {
                con.Open();
            }
            ...
            List<M_SRDetail> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_SRDetail>();
                foreach (DataRow row in dt.Rows)
                {
                    var fields = new M_SRDetail
                    {
        ...
        public M_SRHeader GetSingle(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
...
(select name from currency  where id = currency) as currency_name,
(select name from PaymentMode where value = LTRIM(RTRIM(Payment_Mode))) as Payment_Mode_name,
(select name from salestype where value = LTRIM(RTRIM(Sales_type))) as Sales_type_name from SRHEADER  where   " + whereCondition;
            sqldr = dp.ExecuteReader(con, strsql);
            M_SRHeader fields = null;
            fields = new M_SRHeader();
            if (sqldr.Read())
        ...
        public string getUpdateMasterCount(string yearmonth, M_SRHeader M)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
	 ...
	 select  isnull(max(cast(right(INVOICECODE, 4) as bigint)),1000)	  as code 
        ...
        public string getUpdateMasterCount(string WHERE)
        {
            ...
            strsql = @"SELECT ISNULL(MAX(CAST(INVOICECODE AS bigint)),0)   as code   FROM SRHEADER WHERE " + WHERE;
        ...
        public int Delete_Voucher_for_Edit_mode(string where)
        {
            ...
            strsql = "delete  from SRDETAIL where " + where;
            SqlCommand cmd = new SqlCommand(strsql, con);
        ...
        public void Set_c(int Mode, M_SRHeader m)
        {
            ...
            strsql = "Execute SP_SRHEADER " + Mode.ToString() + ",";
        ...
        public void Set_d(int Mode, M_SRDetail m)
        {
            ...
            strsql = "Execute SP_SRDETAIL " + Mode.ToString() + ",";
