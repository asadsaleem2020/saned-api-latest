   ...
   public class D_DB
    {
        ...
        public List<M_SHOLDHEADER> GetList(string whereCondition)
        {
            ...
            strsql = "select * from SALEHOLDHEADER where   " + whereCondition;

            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            List<M_SHOLDHEADER> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_SHOLDHEADER>();
                foreach (DataRow sqldr in dt.Rows)
                {
                    var fields = new M_SHOLDHEADER
                    {
        ...
        public M_SHOLDHEADER GetSingleContract(string whereCondition)
        {
...
from sale_contract_header where   " + whereCondition;
            ...
            M_SHOLDHEADER fields = null;
            fields = new M_SHOLDHEADER();
            if (sqldr.Read())
            {
        ...
        public List<M_SHOLDDETAIL> GetDetailRowForEdit(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            ...
            strsql = "select * from SALEHOLDDETAIL where   " + whereCondition;
            if (con.State == ConnectionState.Closed)
            {
                con.Open();
            }
            ...
            List<M_SHOLDDETAIL> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_SHOLDDETAIL>();
                foreach (DataRow row in dt.Rows)
                {
                    var fields = new M_SHOLDDETAIL
                    {
        ...
        public M_SHOLDHEADER GetSingle(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
...
(select name from currency  where id = currency) as currency_name,
(select name from PaymentMode where value = LTRIM(RTRIM(Payment_Mode))) as Payment_Mode_name,
(select name from salestype where value = LTRIM(RTRIM(Sales_type))) as Sales_type_name from SALEHOLDHEADER  where   " + whereCondition;
            sqldr = dp.ExecuteReader(con, strsql);
            M_SHOLDHEADER fields = null;
            fields = new M_SHOLDHEADER();
            if (sqldr.Read())
        ...
        public string getUpdateMasterCount(string yearmonth, M_SHOLDHEADER M)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
        ...
        public string getUpdateMasterCount(string WHERE)
        {
            ...
            strsql = @"SELECT ISNULL(MAX(CAST(INVOICECODE AS bigint)),0)   as code   FROM SALEHOLDHEADER WHERE " + WHERE;
        ...
        public int Delete_Voucher_for_Edit_mode(string where)
        {
            ...
            strsql = "delete  from SALEHOLDDETAIL where " + where;
            SqlCommand cmd = new SqlCommand(strsql, con);
        ...
        public void Set_c(int Mode, M_SHOLDHEADER m)
        {
            ...
            strsql = "Execute SP_HOLDEHEADER " + Mode.ToString() + ",";
        ...
        public void Set_d(int Mode, M_SHOLDDETAIL m)
        {
            ...
            strsql = "Execute SP_HOLDDETAIL " + Mode.ToString() + ",";
