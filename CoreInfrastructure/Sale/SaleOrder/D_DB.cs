...
namespace CoreInfrastructure.Sale.SaleOrder
{
    public class D_DB
    {
        ...
        public List<M_OrderHeader> GetList(string whereCondition)
        {
            ...
            strsql = "select * from SOHEADER where   " + whereCondition;

            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            List<M_OrderHeader> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_OrderHeader>();
                foreach (DataRow sqldr in dt.Rows)
                {
                    var fields = new M_OrderHeader
                    {
        ...
        public M_OrderHeader GetSingleContract(string whereCondition)
        {
...
from sale_contract_header where   " + whereCondition;
            ...
            M_OrderHeader fields = null;
            fields = new M_OrderHeader();
            if (sqldr.Read())
            {
        ...
        public List<M_OrderDetail> GetDetailRowForEdit(string whereCondition)
        {
            DataProcess dp = new DataProcess();

            string strsql;
            
            strsql = "select * from SODETAIL where   " + whereCondition;
            if (con.State == ConnectionState.Closed)
            {
                con.Open();
            }
            ...
            List<M_OrderDetail> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_OrderDetail>();
                foreach (DataRow row in dt.Rows)
                {
                    var fields = new M_OrderDetail
                    {
        ...
        public M_OrderHeader GetSingle(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
...
(select name from currency  where id = currency) as currency_name,
(select name from PaymentMode where value = LTRIM(RTRIM(Payment_Mode))) as Payment_Mode_name,
(select name from salestype where value = LTRIM(RTRIM(Sales_type))) as Sales_type_name from SOHEADER  where   " + whereCondition;
            sqldr = dp.ExecuteReader(con, strsql);
            M_OrderHeader fields = null;
            fields = new M_OrderHeader();
            if (sqldr.Read())
        ...
        public string getUpdateMasterCount(string yearmonth, M_OrderHeader M)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
	 ...
	 select  isnull(max(cast(right(INVOICECODE, 4) as bigint)),1000)	  as code 
        ...
        public string getUpdateMasterCount(string WHERE)
        {
            ...
            strsql = @"SELECT ISNULL(MAX(CAST(INVOICECODE AS bigint)),0)   as code   FROM SOHEADER WHERE " + WHERE;
        ...
        public int Delete_Voucher_for_Edit_mode(string where)
        {
            ...
            strsql = "delete  from SODETAIL where " + where;
            SqlCommand cmd = new SqlCommand(strsql, con);
        ...
        public void Set_c(int Mode, M_OrderHeader m)
        {
            ...
            strsql = "Execute SP_SOHEADER " + Mode.ToString() + ",";
        ...
        public void Set_d(int Mode, M_OrderDetail m)
        {
            ...
            strsql = "Execute SP_SODETAIL " + Mode.ToString() + ",";
