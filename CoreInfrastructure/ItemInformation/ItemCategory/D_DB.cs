...
namespace CoreInfrastructure.ItemInformation.ItemCategory
{
    public class D_DB
    {
        ...
        public List<M_ItemCategory> GetList(string whereCondition)
        {
            ...
            strsql = "select * from ItemCategory where   " + whereCondition;
        ...
        public M_ItemCategory GetSingle(string whereCondition)
        {
            ...
            M_ItemCategory fields = null;
            if (sqldr.Read())
            {
                fields = new M_ItemCategory();
            //    fields.ID = int.Parse(sqldr["ID"].ToString());
                fields.Code = sqldr["Code"].ToString();
                fields.Name = sqldr["Name"].ToString();
                fields.Locked = bool.Parse(sqldr["Locked"].ToString());
        ...
        public string getUpdateMasterCount()
        {
            ...
            strsql = @"SELECT ISNULL(MAX( CAST(CODE AS BIGINT  ))  ,0) +1  AS code FROM ItemCategory";
        ...
        public void Set_b(int Mode, M_ItemCategory m)
        {
            ...
            strsql = "Execute SP_ItemCategory " + Mode.ToString() + ",";
