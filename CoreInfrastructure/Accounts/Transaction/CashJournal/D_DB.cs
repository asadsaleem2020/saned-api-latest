...
namespace CoreInfrastructure.Accounts.Transaction.CashJournal
{
    public class D_DB
    {
	 ...
	 select  isnull(max(cast(right(voucherid, 4) as bigint)),1000)	  as code 
        ...
        public string getUpdateMasterCount(string WHERE)
        {
            ...
            strsql = @"SELECT ISNULL(MAX(CAST(VoucherId AS bigint)),0)   as code   FROM Cashjournal_header WHERE " + WHERE;
        ...
        public M_Header GetSingle(string whereCondition)
        {
            ...
            strsql = "select * from Cashjournal_header where   " + whereCondition;
            sqldr = dp.ExecuteReader(con, strsql);
        ...
        public List<M_Detail> GetDetailRowForEdit(string whereCondition)
        {
            ...
            strsql = "select * from Cashjournal_Detail where   " + whereCondition;
        ...
        public int Delete_Voucher_for_Edit_mode(string where)
        {
            ...
            strsql = "delete  from CashJournal_Detail where " + where;
            SqlCommand cmd = new SqlCommand(strsql, con);
