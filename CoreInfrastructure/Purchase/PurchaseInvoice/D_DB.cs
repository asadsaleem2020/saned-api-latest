...
namespace CoreInfrastructure.Purchase.PurchaseInvoice
{
    public class D_DB
    {
        ...
        public List<M_PurchaseHeader> GetList(string whereCondition)
        {
            ...
            strsql = "select * from PURCHASEHEADER where   " + whereCondition;

            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            List<M_PurchaseHeader> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_PurchaseHeader>();
                foreach (DataRow sqldr in dt.Rows)
                {
                    var fields = new M_PurchaseHeader
                    {
        ...
        public M_PurchaseHeader GetSingleContract(string whereCondition)
        {
...
from sale_contract_header where   " + whereCondition;
            ...
            M_PurchaseHeader fields = null;
            fields = new M_PurchaseHeader();
            if (sqldr.Read())
            {
        ...
        public List<M_PurchaseDetail> GetDetailRowForEdit(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            ...
            strsql = "select * from PURCHASEDETAIL where   " + whereCondition;
            if (con.State == ConnectionState.Closed)
            {
                con.Open();
            }
            ...
            List<M_PurchaseDetail> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_PurchaseDetail>();
                foreach (DataRow row in dt.Rows)
                {
                    var fields = new M_PurchaseDetail
                    {
        ...
        public M_PurchaseHeader GetSingle(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
...
(select name from currency  where id = currency) as currency_name,
(select name from PaymentMode where value = LTRIM(RTRIM(Payment_Mode))) as Payment_Mode_name,
(select name from salestype where value = LTRIM(RTRIM(Sales_type))) as Sales_type_name from PURCHASEHEADER  where   " + whereCondition;
            sqldr = dp.ExecuteReader(con, strsql);
            M_PurchaseHeader fields = null;
            fields = new M_PurchaseHeader();
            if (sqldr.Read())
        ...
        public string getUpdateMasterCount(string yearmonth, M_PurchaseHeader M)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
        ...
        public string getUpdateMasterCount(string WHERE)
        {
            ...
            strsql = @"SELECT ISNULL(MAX(CAST(INVOICECODE AS bigint)),0)   as code   FROM PURCHASEHEADER WHERE " + WHERE;
        ...
        public int Delete_Voucher_for_Edit_mode(string where)
        {
            ...
            strsql = "delete  from PURCHASEDETAIL where " + where;
            SqlCommand cmd = new SqlCommand(strsql, con);
        ...
        public void Set_c(int Mode, M_PurchaseHeader m)
        {
            ...
            var str = "yyyy-MM-dd";
            DateTime invoicedate , cdate , udate , ddelete;

            DateTime.TryParse(m.INVOICEDATE.ToString(), out invoicedate);
            DateTime.TryParse(m.CREATED_ON.ToString(), out cdate); 
            DateTime.TryParse(m.UPDATED_ON.ToString(), out udate);
            DateTime.TryParse(m.DELETED_ON.ToString(), out ddelete);
 
             string stringDate = DateTime.Now.ToString("yyyy-MM-dd");
            //string stringDate = m.INVOICEDATE.ToString("yyyy-MM-dd");
            //invoicedate = datestring(m.INVOICEDATE.ToString());
            //cdate = datestring(m.CREATED_ON.ToString());
            //cdate = datestring(m.UPDATED_ON.ToString());
            //ddelete = datestring(m.DELETED_ON.ToString());


            strsql = "Execute SP_PURCHASEHEADER " + Mode.ToString() + ",";
            strsql += "'" + m.COMPANY_CODE.Replace("'", "''") + "',";
            strsql += "'" + m.INVOICECODE.Replace("'", "''") + "',";
            strsql += "'" + m.REF_NO.Replace("'", "''") + "',";
            strsql += "'" + stringDate + "',";
            // strsql += "'2015-06-24',";
        ...
        public string datestring(string stts)
        {
            string parsed_date = "";
            parsed_date = String.Format("{0:yyyy-dd-MMM}", stts);   
            return parsed_date;
        }
        public void Set_d(int Mode, M_PurchaseDetail m)
        {
            ...
            strsql = "Execute SP_PURCHASEDETAIL " + Mode.ToString() + ",";
