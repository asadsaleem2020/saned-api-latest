...
namespace CoreInfrastructure.Purchase.PurchaseOrder
{
    public class D_DB
    {
        ...
        public List<M_POHeader> GetList(string whereCondition)
        {
            ...
            strsql = "select * from POHEADER where   " + whereCondition;

            SqlCommand cmd = new SqlCommand(strsql, con);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            List<M_POHeader> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_POHeader>();
                foreach (DataRow sqldr in dt.Rows)
                {
                    var fields = new M_POHeader
                    {
        ...
        public M_POHeader GetSingleContract(string whereCondition)
        {
...
from sale_contract_header where   " + whereCondition;
            ...
            M_POHeader fields = null;
            fields = new M_POHeader();
            if (sqldr.Read())
            {
        ...
        public List<M_PODetail> GetDetailRowForEdit(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            ...
            strsql = "select * from PODETAIL where   " + whereCondition;
            if (con.State == ConnectionState.Closed)
            {
                con.Open();
            }
            ...
            List<M_PODetail> list = null;
            if (dt.Rows.Count > 0)
            {
                list = new List<M_PODetail>();
                foreach (DataRow row in dt.Rows)
                {
                    var fields = new M_PODetail
                    {
        ...
        public M_POHeader GetSingle(string whereCondition)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
...
(select name from salestype where value = LTRIM(RTRIM(Sales_type))) as Sales_type_name from PURCHASEHEADER  where   " + whereCondition;
        ...
        public string getUpdateMasterCount(string yearmonth, M_POHeader M)
        {
            DataProcess dp = new DataProcess();
            SqlDataReader sqldr;
            string strsql;
	 ...
	 select  isnull(max(cast(right(INVOICECODE, 4) as bigint)),1000)	  as code 
        ...
        public string getUpdateMasterCount(string WHERE)
        {
            ...
            strsql = @"SELECT ISNULL(MAX(CAST(INVOICECODE AS bigint)),0)   as code   FROM POHEADER WHERE " + WHERE;
        ...
        public int Delete_Voucher_for_Edit_mode(string where)
        {
            ...
            strsql = "delete  from PODETAIL where " + where;
            SqlCommand cmd = new SqlCommand(strsql, con);
        ...
        public void Set_c(int Mode, M_POHeader m)
        {
            ...
            strsql = "Execute SP_POHEADER " + Mode.ToString() + ",";
        ...
        public void Set_d(int Mode, M_PODetail m)
        {
            ...
            strsql = "Execute SP_PODETAIL " + Mode.ToString() + ",";
